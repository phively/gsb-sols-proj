...
# Data setup
mderived <- mderived %>% mutate(Actual.Amt.lt = Actual.Amt %>% log_censor(base=10))
# Random sample of dataset
lmdat <- mderived %>% filter(Actual.Amt > 0)
set.seed(1053622)
rand <- sample(1:nrow(lmdat), size=nrow(lmdat))

# Model
lm.sol.close <- lmdat %>% lm(Actual.Amt.lt ~ lt.Planned.Amt + lt.Expected.Amt + Sol.Type.Agg, data=.)
summary(lm.sol.close)

lm.sol.preds <- predict(lm.sol.close, newdata=mderived)
#plot(mderived$Actual.Amt.lt ~ lm.sol.preds); abline(0,1)
```