...
# Plan model, main effects only
glm.pln.t <- glm(FY.Plan.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + Plan.Future + FacMonth(planning.fiscal.mo), data=mderived, family=binomial())
# Clear model, main effects only
glm.clr.t <- glm(FY.Clear.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + Clear.Future + FacMonth(clear.fiscal.mo), data=mderived, family=binomial())
# Ask model, main effects only
glm.ask.t <- glm(FY.Ask.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + LogTrans(Ask.Amt) + Ask.Future + FacMonth(ask.fiscal.mo), data=mderived, family=binomial())
# Oral model, main effects only
glm.ora.t <- glm(FY.Oral.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + LogTrans(Ask.Amt) + Oral.Future + FacMonth(oral.fiscal.mo), data=mderived, family=binomial())
# Put together the error rates
...
rbind(dat2, CalcErrorRates(ConfusionMatrix(list(
    update(glm.pln.t, . ~ . -LogTrans(Expected.Amt)),
    update(glm.clr.t, . ~ . -LogTrans(Expected.Amt)),
    update(glm.ask.t, . ~ . -LogTrans(Expected.Amt)),
    update(glm.ora.t, . ~ . -LogTrans(Expected.Amt))
...
```{r, message=F, warning=F, cache=F}
# Plan model, main effects only
glm.pln.t <- glm(FY.Plan.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + Plan.Future, data=mderived, family=binomial())
# Clear model, main effects only
glm.clr.t <- glm(FY.Clear.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + Clear.Future, data=mderived, family=binomial())
# Ask model, main effects only
glm.ask.t <- glm(FY.Ask.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + LogTrans(Ask.Amt) + Ask.Future, data=mderived, family=binomial())
# Oral model, main effects only
glm.ora.t <- glm(FY.Oral.Book ~ Sol.Type.Agg + LogTrans(Planned.Amt) + LogTrans(Expected.Amt) + LogTrans(Ask.Amt) + Oral.Future, data=mderived, family=binomial())
# Put together the error rates
confuse <- ConfusionMatrix(list(glm.pln.t, glm.clr.t, glm.ask.t, glm.ora.t), modnames=modnames, counts=F)
# Row to add to the error table
...
* These error rates and models definitely need to be compared to JMod in its native environment, namely predicted dollar amounts
