...
confusion <- function(models, threshold, modnames, debug=F){
  out <- list()
  # Check if not actually passed a list of models
  is.list <- class(models)[1]=="list"
    if(debug){print(is.list)}
  ...
  for(i in 1:n){
    ...
    out[[i]] <- table(truth=model.frame(m)[,1], pred=fitted(m)>=threshold)
  }
  names(out) <- modnames
  return(out)
}
...
detach(package:MASS) #clean up because MASS conflicts with dplyr
```

Compare confusion matrices for JMod versus the month intercepts logistic model by stage.

```{r, message=F, warning=F, cache=F}
print("Logistic models")
confusion(list(glm.pln.1, glm.clr.1, glm.ask.1, glm.ora.1), threshold=.5, modnames=modnames, debug=F)
```
...
source("f.JMod.R")
print("JMod")
JMod(curr.stage=1, curr.dt=mdat$Planning.Dt, expected.dt=mdat$Expected.Dt)$probability
```
