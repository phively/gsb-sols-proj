...
Compare errors for JMod versus the month intercepts logistic model by stage. The error function is
...
print("Clear")
data.frame(JMod=sum(abs(JMod.err(probs=JMod(curr.stage=2, curr.dt=mdat$Clear.Dt, expected.dt=mdat$Expected.Dt)$probability, truth=mdat$FY.Clear.Book)), na.rm=T), glm=sum(abs(JMod.err(probs=glm.clr.1$fitted, truth=mdat$FY.Clear.Book)), na.rm=T))
print("Ask")
data.frame(JMod=sum(abs(JMod.err(probs=JMod(curr.stage=3, curr.dt=mdat$Ask.Dt, expected.dt=mdat$Expected.Dt)$probability, truth=mdat$FY.Ask.Book)), na.rm=T), glm=sum(abs(JMod.err(probs=glm.ask.1$fitted, truth=mdat$FY.Ask.Book)), na.rm=T))
print("Oral")
data.frame(JMod=sum(abs(JMod.err(probs=JMod(curr.stage=4, curr.dt=mdat$Oral.Dt, expected.dt=mdat$Expected.Dt)$probability, truth=mdat$FY.Oral.Book)), na.rm=T), glm=sum(abs(JMod.err(probs=glm.ora.1$fitted, truth=mdat$FY.Oral.Book)), na.rm=T))
```
