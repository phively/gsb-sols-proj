...
head.all <- names(mderived)[names(mderived) %in% c("Solicitation.Type.Desc", "Final.Sol.Mgr", "Planned.Fiscal.Mo", "Planned.Amt", "Expected.Fiscal.Mo", "Expected.Amt", "Planned.Ask.Band")]
...
head.plan <- setdiff(head.plan, "plan2clear") #take plan2clear which wouldn't have been known
# Look for "Clear" in header
head.clear <- names(mderived)[setdiff(grep("[cC]lear", names(mderived)), grep("FY.", names(mderived)))]
head.clear <- setdiff(head.clear, "clear2ask") #take out clear2ask which wouldn't have been known
# Look for "Ask" in header
head.ask <- names(mderived)[setdiff(grep("[aA]sk", names(mderived)), grep("FY.", names(mderived)))]
head.ask <- setdiff(head.ask, "ask2oral") #take out ask2oral which wouldn't have been known
...
treedat$FY.Plan.Book <- as.factor(treedat$FY.Plan.Book)
rf.plan <- randomForest(x=treedat[,-1], y=treedat[,1])
# Clear stage forest
treedat <- na.omit(mderived[,c("FY.Clear.Book", head.all, head.clear)])
treedat$FY.Clear.Book <- as.factor(treedat$FY.Clear.Book)
rf.clear <- randomForest(x=treedat[,-1], y=treedat[,1])
# Ask stage forest
treedat <- na.omit(mderived[,c("FY.Ask.Book", head.all, head.ask)])
treedat$FY.Ask.Book <- as.factor(treedat$FY.Ask.Book)
rf.ask <- randomForest(x=treedat[,-1], y=treedat[,1])
# Oral stage forest
treedat <- na.omit(mderived[,c("FY.Oral.Book", head.all, head.oral)])
treedat$FY.Oral.Book <- as.factor(treedat$FY.Oral.Book)
rf.oral <- randomForest(x=treedat[,-1], y=treedat[,1])
```
