...
```{r, echo=F, warning=F, cache=F}
ggdat <- na.omit(mdat %>% mutate(Count=1) %>% group_by(FY.plan, FY.clear, FY.ask, FY.oral) %>% summarise(Count=sum(Count)) %>% gather("Stage","Closed",1:4,factor_key=T) %>% group_by(Stage, Closed) %>% summarise(Count=sum(Count)))
...
```{r, echo=F, warning=F, cache=F}
ggdat <- na.omit(mdat %>% mutate(Count=1) %>% group_by(FY.Plan.Book, FY.Clear.Book, FY.Ask.Book, FY.Oral.Book) %>% summarise(Count=sum(Count)) %>% gather("Stage","Booked.In.FY",1:4,factor_key=T) %>% group_by(Stage, Booked.In.FY) %>% summarise(Count=sum(Count)))
levels(ggdat$Stage) <- c("Plan","Clear","Ask","Oral")
# Plot of counts
