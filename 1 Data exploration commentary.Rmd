...
#### Ask Amount Different From Planned Amount
...
# Look at relative frequencies
g2 <- ggplot(mdat, aes(x=(Ask.Amt/Planned.Amt), y=..count.., color=Booked, fill=Booked)) + geom_hline(yintercept=.5, alpha=.5) + geom_density(alpha=.5, position="fill") + labs(x="Ask amount", y="Count", title="Close rates by Ask/Planned") + scale_x_continuous(trans="log10", labels=scales::percent)
# Final layout
grid.arrange(g1, g2)
```
...
g1 <- ggplot(mdat, aes(x=Ask.Amt/Planned.Amt, color=Booked, fill=Booked)) + geom_histogram(alpha=.5, breaks=bins) + labs(x="Ask amount", y="Count", title="Close counts by Ask/Planned") + scale_x_continuous(labels=scales::percent, breaks=c(seq(0,1,by=.25), seq(1,max(bins),by=.5)))
# Look at relative frequencies
g2 <- ggplot(mdat, aes(x=Ask.Amt/Planned.Amt, color=Booked, fill=Booked)) + geom_hline(yintercept=.5, alpha=.5) + geom_histogram(alpha=.5, position="fill", breaks=bins) + labs(x="Ask amount", y="Count", title="Close rates by Ask/Planned") + scale_x_continuous(labels=scales::percent, breaks=c(seq(0,1,by=.25), seq(1,max(bins),by=.5)))
# Final layout
grid.arrange(g1, g2)
```

If someone asks slightly more than the planned ask amount it's a good sign that the talks were going well; otherwise there doesn't seem to be a lot to see here.
