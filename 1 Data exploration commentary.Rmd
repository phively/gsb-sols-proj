...
Peak ask occurs around Nov-Dec and Mar each year. Not nearly as much oral pledge data; usually oral to booked is fast enough that the date isn't captured in the system. (The current model rolls ask and oral together.)
...
This is fascinating; a solicitation entering plan or clear in December or plan in April-May is more likely to be refused rather than booked. Is there a specific year driving that?

```{r, echo=F, warning=F, cache=T}
## Redo previous plots for each fiscal year
tmp <- mdat %>% select(Planning.Dt, Clear.Dt, Ask.Dt, Oral.Dt, Booked) %>%
  mutate(Count=1, PlanY=as.numeric(year(Planning.Dt)), Plan=as.numeric(month(Planning.Dt)),
...
ggplot(ggdat[ggdat$Year!="2010",], aes(x=Month, y=Count, color=Stage, fill=Stage, group=Stage)) + geom_point() + geom_line() + labs(y="Count of solicitations entering stage by year") + facet_grid(Year~.) + theme(text=element_text(size=12))
```

2010, 2016, 2017 don't have enough data so they're dropped. 2011 didn't have much stage progress in the second half of the year.
...
```{r, echo=F, warning=F, cache=T}
g1 <- ggplot(mdat, aes(x=Ask.Amt, y=..count.., color=Booked, fill=Booked)) + geom_density(alpha=.5) + labs(x="Ask amount", y="Count", title=expression(paste("Close counts by ",Log[10]," ask amount"))) + scale_x_continuous(trans="log10", breaks=10^seq(1:7), labels=scales::dollar) #+ facet_grid(Booked~.)
...
Above 50% close rate at every level, but the poorest close rate is between the $100k to $1M levels. There's obviously nonlinearity; a cubic polynomial might be a good approximation?
...
# Look at relative frequencies
g2 <- ggplot(mdat, aes(x=(Ask.Amt/Planned.Amt), y=..count.., color=Booked, fill=Booked)) + geom_hline(yintercept=.5, alpha=.5) + geom_density(alpha=.5, position="fill") + labs(x="Ask amount", y="Probability", title="Close rates by Ask/Planned") + scale_x_continuous(trans="log10", labels=scales::percent)
# Final layout
grid.arrange(g1, g2)
```
...
# Look at relative frequencies
g2 <- ggplot(mdat, aes(x=Ask.Amt/Planned.Amt, color=Booked, fill=Booked)) + geom_hline(yintercept=.5, alpha=.5) + geom_histogram(alpha=.5, position="fill", breaks=bins) + labs(x="Ask amount", y="Probability", title="Close rates by Ask/Planned") + scale_x_continuous(labels=scales::percent, breaks=c(seq(0,1,by=.25), seq(1.5,max(bins),by=.5)))
# Final layout
grid.arrange(g1, g2)
```
...
ggplot(ggdat, aes(x=Stage, y=Proportion, color=Closed, fill=Closed)) + geom_point(stat="identity", alpha=.5, size=2) + geom_linerange(aes(ymin=Proportion-ci, ymax=Proportion+ci), alpha=.25, size=3) + labs(y="Probability") + scale_y_continuous(limits=c(0,1), breaks=seq(0,1,by=.2))
...
# Plot of probabilities
ggdat <- ggdat %>% mutate(n=sum(Count), Proportion=Count/n, ci=sqrt(Proportion*(1-Proportion)/n))
ggplot(ggdat, aes(x=Stage, y=Proportion, color=Booked.In.FY, fill=Booked.In.FY)) + geom_point(stat="identity", alpha=.5, size=2) + geom_linerange(aes(ymin=Proportion-ci, ymax=Proportion+ci), alpha=.25, size=3) + labs(y="Probability") + scale_y_continuous(limits=c(0,1), breaks=seq(0,1,by=.2))
...
There are a couple of crossover points; January for Plan and Clear; Jan or Feb for ask (though it crosses back in March interestingly). Can't count on oral pledges made in May or June coming in before the end of the year. Let's restrict the plot to just asks in the MG range, $x\in[100k,5M)$:
